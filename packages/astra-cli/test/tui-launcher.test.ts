...
test("builds a fail-closed read-only TUI launch", () => {
  const spec = makeAstraTuiLaunchSpec("/workspace", "read-only", authority)
...
test("keeps every inherited tool blocked after Activate once", () => {
  const permission = JSON.parse(
    makeAstraTuiLaunchSpec("/workspace", "activate-once", authority).env.OPENCODE_PERMISSION!,
  )
...
test("refuses to create an authority after the workspace becomes stale", async () => {
  ...
  try {
    await expect(
      createAstraSessionAuthorityFile(
    ...
    ).rejects.toThrow("workspace authority became stale")
  } finally {
