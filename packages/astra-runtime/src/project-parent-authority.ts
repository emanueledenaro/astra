...
export async function captureProjectParentAuthority(
  ...
  const firstParent = await inspectParent(parentPath)
  ...
  const firstTarget = await inspectTarget(targetPath)
  if (firstTarget === "present") return blocked("target_already_exists")
  if (firstTarget === "unreadable") return blocked("target_unreadable")

  const finalParent = await inspectParent(parentPath)
  if (!finalParent.ok || !sameIdentity(firstParent.facts, finalParent.facts)) {
    return blocked("parent_identity_changed")
  }
  if ((await safeRealpath(parentPath)) !== parentPath) return blocked("parent_identity_changed")
  const finalTarget = await inspectTarget(targetPath)
...
export async function revalidateProjectParentAuthority(
  ...
  const firstParent = await inspectParent(authority.parentPath)
  if (!firstParent.ok) return blocked(firstParent.reason)
  if (!sameAuthorityIdentity(firstParent.facts, authority)) return blocked("parent_identity_changed")
  if ((await safeRealpath(authority.parentPath)) !== authority.parentPath) return blocked("parent_not_canonical")
  const firstTarget = await inspectTarget(authority.targetPath)
  if (firstTarget === "present") return blocked("target_appeared")
  if (firstTarget === "unreadable") return blocked("target_unreadable")

  const finalParent = await inspectParent(authority.parentPath)
  if (
    !finalParent.ok ||
  ...
  if ((await safeRealpath(authority.parentPath)) !== authority.parentPath) return blocked("parent_identity_changed")
  const finalTarget = await inspectTarget(authority.targetPath)
...
async function inspectTarget(path: string): Promise<"absent" | "present" | "unreadable"> {
  try {
    await lstat(path)
    return "present"
  } catch (cause) {
