...
export type AstraHostCommandClient = Readonly<{
  prepare: (script: string, options?: RequestOptions) => Promise<HostCommandPrepareResult>
  decide: (
    proposalID: string,
    decision: "approve" | "reject",
    options?: RequestOptions & Readonly<{ onProgress?: (progress: HostCommandProgress) => void }>,
  ) => Promise<HostCommandDecisionResult>
  dispose: () => void
}>

/** Creates a lazy client whose child-controlled prepare payload is only one bounded script. */
export function createAstraHostCommandClient(
  environment: Readonly<Record<string, string | undefined>>,
  sessionID: string,
  options: Readonly<{ responseTimeoutMs?: number; expectedWorkspaceRoot?: string }> = {},
): AstraHostCommandClient {
  ...
  return {
    prepare(script, requestOptions = {}) {
      if (!validScript(script)) return Promise.reject(new AstraControlClientError("protocol_invalid"))
      return request({
...
type ExchangeInput<Result> = Readonly<{
  method: "host-command.prepare" | "host-command.decide"
  script?: string
  proposalID?: string
  decision?: "approve" | "reject"
...
function exchange<Result>(input: CompleteExchangeInput<Result>, registerCancel: (cancel: () => void) => void) {
  return new Promise<Result>((complete, reject) => {
    ...
    let buffered = ""
    let progressIndex = 0
    let observation: Readonly<{ receiptID: string; outputDigest: string }> | undefined
    ...
    socket.once("connect", () => {
      socket?.write(
        JSON.stringify({
          ...
          token: input.token,
          ...(input.method === "host-command.prepare"
            ? { script: input.script }
    ...
    socket.on("data", (chunk: string) => {
      ...
      for (const line of lines) {
        ...
        if (message.type === "progress") {
          if (
            input.method !== "host-command.decide" ||
          ...
          if (message.progress.status === "effect_observed_not_verified") {
            observation = { receiptID: message.progress.receiptID, outputDigest: message.progress.outputDigest }
          }
          try {
...
function validTerminalFlow<Result>(
  ...
  observation: Readonly<{ receiptID: string; outputDigest: string }> | undefined,
) {
  const record = plainRecord(result)
  if (!record || record.requestId !== input.requestID) return false
  if (input.method === "host-command.prepare") return progressIndex === 0
  ...
  return (
    ...
    output?.digestScope === "stdout_stderr_exit"
  )
}

type ControlMessage<Result> =
  | Readonly<{ type: "accepted" }>
  | Readonly<{ type: "progress"; progress: HostCommandProgress }>
...
function parseMessage<Result>(
  ...
  try {
    ...
    if (record.type === "host-command.progress") {
      if (!exactKeys(record, ["schemaVersion", "type", "requestId", "progress"])) return null
      const parsed = parseHostCommandProgress(record.progress)
      return parsed.ok && parsed.value.requestId === requestID ? { type: "progress", progress: parsed.value } : null
    }
    if (record.type !== "host-command.terminal" || !exactKeys(record, ["schemaVersion", "type", "requestId", "result"]))
      return null
